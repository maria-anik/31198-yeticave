<?php
    ...
    if ($con && !empty($search_word)) {
        ...
        $result = mysqli_query($con, "  SELECT COUNT(*) as cnt
                                        FROM lots_list
                                        WHERE MATCH (title , description) AGAINST ('$search_word')
                                        OR CONCAT(title, description) LIKE '%$search_word%'
                                        AND NOW()<date_end ");
        ...
        $sql_search = " SELECT l.id,  l.title , c.title as category_name, category, date_end, cost, img, img_alt, description
                        FROM lots_list l
                        JOIN categories c
                        ON l.category_id=c.id
                        WHERE (MATCH (l.title , l.description) AGAINST ('$search_word')
                        OR CONCAT(l.title, l.description) LIKE '%$search_word%')
                        AND NOW()<date_end
